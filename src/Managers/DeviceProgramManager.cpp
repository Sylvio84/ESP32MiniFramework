...
void DeviceProgramManager::init()
{
    ...
    if (loadDevicePrograms()) {
        debug("Loaded " + String(devicePrograms.size()) + " device programs",0);
    } else {
        debug("Failed to load device programs", 0);
    }
...
void DeviceProgramManager::registerCommands()
{
    ...
    cmdMgr->registerCommand(
        Command("program", "list", "List all device programs with details", CommandSource::Any, true, [this](const std::vector<String>& args) -> String {
    ...
    cmdMgr->registerCommand(
        Command("program", "info", "Show detailed program information", CommandSource::Any, true, [this](const std::vector<String>& args) -> String {
            if (args.size() == 0) {
    ...
    cmdMgr->registerCommand(
        Command("program", "import", "Import program from JSON", CommandSource::Any, false, [this](const std::vector<String>& args) -> String {
            ...
            if (importDeviceProgram(jsonData, errorMsg)) {
                ...
                for (const auto* prog : programs) {
                    if (prog && prog->enabled && prog->program) {
                        ...
                        debug(
                            "ACTIVE: " + prog->name + " [" + devicesStr + "] at " + prog->program->startTime + " for " + String(prog->program->duration) + "s",
                            0);
                    }
                }
                debug("=== END SUMMARY ===", 0);
    ...
    cmdMgr->registerCommand(
        Command("program", "enable", "Enable a device program", CommandSource::Any, true, [this](const std::vector<String>& args) -> String {
            if (args.size() == 0) {
    ...
    cmdMgr->registerCommand(
        Command("program", "disable", "Disable a device program", CommandSource::Any, true, [this](const std::vector<String>& args) -> String {
            if (args.size() == 0) {
    ...
    cmdMgr->registerCommand(
        Command("program", "remove", "Remove a device program", CommandSource::Any, true, [this](const std::vector<String>& args) -> String {
            if (args.size() == 0) {
    ...
    cmdMgr->registerCommand(
        Command("program", "export", "Export program as JSON", CommandSource::Serial, false, [this](const std::vector<String>& args) -> String {
            if (args.size() == 0) {
    ...
    cmdMgr->registerCommand(
        Command("program", "load", "Load programs from storage", CommandSource::Any, true, [this](const std::vector<String>& args) -> String {
    ...
    cmdMgr->registerCommand(Command("program", "save", "Save programs to storage", CommandSource::Any, true, [this](const std::vector<String>& args) -> String {
        if (saveDevicePrograms()) {
